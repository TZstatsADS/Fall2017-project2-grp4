...
shinyServer = function(input, output, session) {
  ...
  output$transit_information = renderText(c(paste('<span style="color:#5F5DA3"> Transit Information: </span>'),paste("<br><b>", 'step ', 1:length(unique(route_df()[[1]]$instruction)), ": ", unique(route_df()[[1]]$instruction), "</b>")))
  ...
  observe({
    ...
    if(is.null(click))
      return()
    review_text = c(paste('<span style="color:#5F5DA3"> Most Recent Review: </span>', paste("<br><b>",ctb()[!duplicated(ctb()[,1]),]$review_text[ctb()[!duplicated(ctb()[,1]),]$id==click$id],"</b>")))
    review_rating = c(paste('<span style="color:#5F5DA3"> Review Rating: </span>') ,paste("<b>",ctb()[!duplicated(ctb()[,1]),]$review_rating[ctb()[!duplicated(ctb()[,1]),]$id==click$id],"</b>"))
    review_time = c(paste('<span style="color:#5F5DA3"> Time of Rating: </span>'), paste("<br><b>",ctb()[!duplicated(ctb()[,1]),]$review_time[ctb()[!duplicated(ctb()[,1]),]$id==click$id],"</b>"))
